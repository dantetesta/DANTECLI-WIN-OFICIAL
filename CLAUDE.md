...
| `ui/`         | `dante_ui` / `dante::ui`               | core/platform/persistence + Qt6::Core | controllers C++ expostos via **context property**: `AppController` (`App`), **`SysStatsController`** (`Sys`, QTimer 2s → CPU%/memMB/cores), **`FilesController`** (`Files`, navegação real de pastas) |
...
## Backlog aberto
- **Sidebar FUNCIONAL (FEITO, verificado no CI):** Pastas = navegação real do filesystem (`FilesController`, árvore lazy 1 nível/vez, `QStandardPaths`, toggle de ocultos, `FolderDialog`); `•••` abre menu de atalhos do sistema; olho alterna ocultos. Ícones do cabeçalho e da árvore são **vetoriais (Canvas), sem emoji**. Pills CPU/RAM da toolbar são **ao vivo** (`Sys`) e abrem o popover **Consumo de Recursos** (gauges + sparkline). Toolbar enxuta: só o mic (🌐/🎭 removidos).
- **Persistência (PENDENTE):** Favoritos/Snippets/Chaves hoje vivem só em `ListModel` em memória → somem no restart. Próximo: persistir via `dante::persistence` (Database/MigrationRunner já existem). Chaves: campos secretos devem ir via DPAPI (lazy), nunca em claro.
- **Mic (PENDENTE):** botão 🎤 vai integrar transcrição via API Whisper do **groq.com**.
